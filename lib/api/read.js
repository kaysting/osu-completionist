...
const getBulkUserProfiles = userIds => {
    ...
    const rows = db
        .prepare(
            ...
            c.name AS country_name, u.default_category
         FROM users u
...
const getAllRecentPasses = (categoryId, limit = 100, offset = 0, after = 0) => {
    // Convert category filters to SQL
    const { where, params, def } = statCategories.categoryToSql(categoryId, 'b');
    ...
    const rows = db
        .prepare(
            `SELECT up.map_id, up.time_passed, up.user_id, up.mode
            FROM user_passes up
            JOIN beatmaps b ON up.map_id = b.id AND up.mode = b.mode
            JOIN users u ON up.user_id = u.id
            LEFT JOIN user_import_queue q ON q.user_id = up.user_id
            WHERE ${where}
                AND up.time_passed > ?
                AND up.time_passed > u.last_import_time
                AND (q.time_started IS NULL OR q.time_started = 0)
            ORDER BY up.time_passed DESC
            LIMIT ? OFFSET ?`
        )
        .all(...params, after, limit, offset);

    // Get full map data
    const beatmapModeMap = {};
    const uniqueMapIds = [];
    for (const row of rows) {
        if (uniqueMapIds.includes(row.map_id)) continue;
        uniqueMapIds.push(row.map_id);
    }
    const modeFilter = def.filters.find(f => f.field === 'mode');
    const modeHint = modeFilter?.equals || null;
    const beatmaps = getBulkBeatmaps(uniqueMapIds, true, modeHint);
    for (const map of beatmaps) {
        if (!map) continue;
        beatmapModeMap[`${map.id}-${map.mode}`] = map;
    }

    // Get user data
    const uniqueUserIds = [];
    for (const row of rows) {
        if (uniqueUserIds.includes(row.user_id)) continue;
        uniqueUserIds.push(row.user_id);
    }
    const usersMap = {};
    const users = getBulkUserProfiles(uniqueUserIds);
    ...
    const passes = [];
    for (const row of rows) {
        const beatmap = beatmapModeMap[`${row.map_id}-${row.mode}`];
        const user = usersMap[row.user_id];
        if (!beatmap || !user) continue;
        passes.push({
            time_passed: row.time_passed,
            beatmap,
            user
...
const getRecentActivityLeaderboard = (categoryId, limit = 10, after = 0) => {
    // 1. Convert filters to SQL
    const { where, params, def } = statCategories.categoryToSql(categoryId, 'b');

    // 2. Fetch Aggregated Stats
    // We group by user_id to sum up the duration of all maps they passed
    // Note: I am assuming your beatmap column for duration is 'total_length'.
    // If it is 'length' or 'hit_length', swap that word in the SUM() function.
    const rows = db
        .prepare(
            `SELECT 
                up.user_id, 
                SUM(b.duration_secs) as total_duration,
                COUNT(*) as play_count
            FROM user_passes up
            JOIN beatmaps b ON up.map_id = b.id AND up.mode = b.mode
            JOIN users u ON up.user_id = u.id
        ...
        .all(...params, after, limit);

    if (rows.length === 0) return [];

    const uniqueUserIds = rows.map(r => r.user_id);
    const users = getBulkUserProfiles(uniqueUserIds);
    const usersMap = {};
    for (const user of users) {
        if (!user) continue;
    ...
    for (const row of rows) {
        ...
        leaderboard.push({
            rank: i,
            user,
            secs: row.total_duration,
            xp: secsToXp(row.total_duration),
...
const searchBeatmaps = (query, category, sort, notPlayedByUserId, limit = 50, offset = 0) => {
    const filterRegex =
        /(cs|ar|od|hp|keys|key|stars|star|sr|bpm|length|mode|year|month|status)\s?(<=|>=|=|<|>)\s?(?:(["'])(.*?)\3|(\S+))(?:\s|$)/gi;
    const filterMatches = query.toLowerCase().matchAll(filterRegex);
    const textQuery = query.toLowerCase().replace(filterRegex, '').trim();
    const params = [];
    const whereClauses = [];
    let joinClause = '';
    let sortClause = '';
    ...
    for (const match of filterMatches) {
        ...
        let rawValue = (match[4] || match[5]).toLowerCase();
        ...
        const applyFloatFilter = col => {
            const { valuesInFloat, valuesBetweenFloat, valueFloat } = sanitizedValue(rawValue);
            ...
            if (operator === '=' && !isNaN(valueFloat) && !rawValue.includes('.')) {
        ...
        const applyStringFilter = col => {
            const { valuesIn } = sanitizedValue(rawValue);
            if (operator === '=' && valuesIn.length > 2) {
        ...
        switch (key) {
            ...
            case 'length':
                ...
                break;
            case 'mode':
                rawValue = utils.rulesetNameToKey(rawValue);
            ...
            case 'status': {
                ...
                rawValue = statusNames[rawValue];
                if (!rawValue) break;
                applyStringFilter('map.status');
                break;
            }
            case 'year': {
                const { valuesBetweenInt, valuesInInt, valueInt } = sanitizedValue(rawValue);
                // Handle year range
            ...
            case 'month': {
                if (!rawValue.match(/^\d{4}-\d{1,2}$/)) break;
        ...
        // Push filter only if it was valid and applied
        filters.push({ key, operator, value: rawValue });
    }
...
const getQueuedUsers = (limit = 100) => {
    const rows = db.prepare(`SELECT * FROM user_import_queue ORDER BY time_queued DESC`).all().splice(0, 100);
