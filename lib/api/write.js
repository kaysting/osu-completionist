...
const savePassesFromScores = async scores => {
    try {
        utils.log(`Checking ${scores.length} scores for relevancy...`);
        ...
        if (mapIds.size === 0) {
            utils.log(`No relevant scores to process.`);
            return true;
        }
        ...
        // Process scores
        utils.log(`Processing and saving passes from ${relevantScoreCount} relevant scores...`);
        const savedPasses = [];
...
const savePassesFromScoreCache = async () => {
    try {
        ...
        // Save passes
        const passesSavedSuccessfully = await savePassesFromScores(scores);
        if (!passesSavedSuccessfully) {
