    ...
    try {
        ...
        while (dbMapIds.length > 0) {
            ...
            const mapRows = db
                .prepare(
                    `
                SELECT map.*, mapset.artist, mapset.title, mapset.status AS mapset_status
                FROM beatmaps map
                JOIN beatmapsets mapset ON map.mapset_id = mapset.id
        ...
        if (missingMapIds.length > 0) {
            utils.log(`Fetching data for ${missingMapIds.length} missing beatmaps...`);
        }
        ...
        for (const mapId in maps) {
            ...
            if (!data.saved && validStatuses.includes(data.mapset_status)) {
                await saveMapset(data.mapset_id, !data.saved);
                countSavedMapsets++;
            }
        }
        ...
        if (savedPasses.length > 0) {
            const content = savedPasses
                .map(pass => {
                    return `-# * **[${pass.userName}](<${env.HTTPS ? 'https' : 'http'}://${env.HOST}/u/${pass.userId}>)** gained **${utils.formatNumber(pass.xp)} cxp** from [${pass.mapName}](<https://osu.ppy.sh/beatmapsets/${pass.mapsetId}#${pass.mode}/${pass.mapId}>) **(${utils.rulesetKeyToName(pass.mode, true)})**`;
                })
                .join('\n');
