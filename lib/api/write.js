...
const updateUserCategoryStats = (userId, force = false) => {
    ...
    try {
        ...
        // Determine and set primary category
        let primaryCatId = 'osu-ranked';
        for (const cat of statCategories.definitions) {
            // Skip if global user
            if (IS_GLOBAL) continue;
            // Skip all global/aggregate modes
            if (cat.id.startsWith('global')) continue;

            const statsCurrent = catStatsNew[cat.id];
            if (!statsCurrent) continue;

            // Run redundancy/significance check
            // The point here is to weed out categories that include loved/converts that
            // aren't different enough from their base counterpart
            // In order for a category to be distinct, it has to have at least minCountDelta more passes
            // AND minPercentDelta more completion than base
            let isDistinct = true;
            const minCountDelta = 5000;
            const minPercentDelta = 0.1;

            // Check loved category
            if (cat.id.includes('-loved')) {
                const baseId = cat.id.replace('-loved', '');
                const statsBase = catStatsNew[baseId];
                const delta = statsCurrent.count_completed - (statsBase?.count_completed || 0);

                if (delta < minCountDelta && delta < statsCurrent.count_completed * minPercentDelta) isDistinct = false;
            }

            // Check converts category
            // Don't bother if we already deemed this category distinct
            if (isDistinct && cat.id.includes('-converts')) {
                const baseId = cat.id.replace('-converts', '');
                const statsBase = catStatsNew[baseId];
                const delta = statsCurrent.count_completed - (statsBase?.count_completed || 0);
            ...
            // Skip if not distinct enough
            if (!isDistinct) continue;

            // If the category is distinct, see if it has higher completion than the current primary
            const statsPrimary = catStatsNew[primaryCatId];
            if (statsCurrent.count_completed > statsPrimary.count_completed) {
                primaryCatId = cat.id;
            }
        }
        if (!IS_GLOBAL && user.is_primary_category_auto && user.primary_category !== primaryCatId) {
            db.prepare(`UPDATE users SET primary_category = ? WHERE id = ?`).run(primaryCatId, user.id);
            utils.log(`Changed ${user.name}'s primary category to ${primaryCatId}`);
        }
    ...
    try {
        ...
        if (savedPasses.length > 0) {
            ...
            for (const pass of savedPasses) {
                const userLink = `[${pass.userName}](<${env.HTTPS ? 'https' : 'http'}://${env.HOST}/u/${pass.userId}>)`;
...
const generateSitemap = async outputPath => {
    ...
    for (const entry of userEntries) {
        userPages.push({
            id: entry.id,
            category_id: entry.primary_category,
