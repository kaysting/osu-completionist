...
const generateSitemap = async (outputPath) => {
    ...
    const users = db.prepare(`SELECT id, last_pass_time, time_created FROM users`).all();
    const categoryIds = statCategories.definitions.map(cat => cat.id);
    const staticPaths = [
        '', 'tos', 'privacy', 'changelog', 'faq', 'search'
    ];
    const xml = await ejs.renderFile(path.join(env.ROOT, 'apps/web/views/sitemap.ejs'), {
        baseUrl: env.BASE_URL, users, categoryIds, staticPaths, dayjs
    });
    fs.writeFileSync(outputPath, xml);
    utils.log(`Generated and saved sitemap to ${outputPath}`);
};
