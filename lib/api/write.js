    ...
    try {
        ...
        for (const cat of statCategories.definitions) {
            ...
            const categoryFields = [
                ...
                { name: 'maps passed', value: utils.formatNumber(statsNew.count_completed), inline: true }
            ];
            ...
            if (totalCxpStepNew > totalCxpStepOld) {
                milestoneEmbeds.push(makeEmbed(
                    `Reached ${utils.formatNumber(totalCxpStepNew)} cxp in ${categoryName}!`,
                    categoryFields
                ));
            }
        ...
        for (const userId in scoresByUser) {
            ...
            const transaction = db.transaction(() => {
                for (const score of scoresByUser[userId]) {
                    ...
                    const existingPass = db.prepare(`SELECT * FROM user_passes WHERE user_id = ? AND map_id = ? AND mode = ? LIMIT 1`).get(userId, mapId, mode);
