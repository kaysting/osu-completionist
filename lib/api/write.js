        ...
        for (const cat of statCategories.definitions) {
            ...
            for (const statsNew of yearlyNew) {
                ...
                if (yearlyPercentStepNew > yearlyPercentStepOld) {
                    milestoneEmbeds.push(makeEmbed(
                        `Reached ${yearlyPercentStepNew}% ${statsNew.year} completion in ${categoryName} (${isThisYear ? 'for now' : ''})!`,
                        fields
                    ));
                }
                ...
                if (yearlyPassCountStepNew > yearlyPassCountStepOld) {
                    milestoneEmbeds.push(makeEmbed(
                        `Reached ${yearlyPassCountStepNew.toLocaleString()} ${statsNew.year} passes in ${categoryName} (${isThisYear ? 'for now' : ''})!`,
                        fields
                    ));
                }
