    ...
    try {
        ...
        while (dbMapIds.length > 0) {
            ...
            const mapRows = db
                ...
                .all(...ids, minRankedTime);
            for (const row of mapRows) {
                const data = {
                    id: row.id,
                    mapset_id: row.mapset_id,
                    map_status: row.status,
                    // Invalidate mapset status on maps ranked before minRankedTime
                    // We'll always fetch these so we can check if they've been unranked
                    mapset_status: row.time_ranked > minRankedTime ? 'unknown' : row.mapset_status,
                    title: row.title,
                    artist: row.artist,
                    name: row.name,
        ...
        // Fetch missing maps or those with invalid statuses
        const missingMapIds = [];
        for (const mapId of mapIds) {
            if (!maps[mapId] || !validStatuses.includes(maps[mapId].mapset_status)) {
        ...
        let countSavedMapsets = 0;
        for (const mapId in maps) {
            const data = maps[mapId];
            if (!data.saved && validStatuses.includes(data.mapset_status)) {
                await saveMapset(data.mapset_id);
                countSavedMapsets++;
            }
        }
        // Update global stats if we saved any mapsets
        if (countSavedMapsets > 0) {
            updateUserCategoryStats(0);
        }
