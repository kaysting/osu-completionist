...
const broadcast = require('#api/broadcast.js');
...
const saveMapsetTransaction = db.transaction((mapset, shouldIndex) => {
    ...
    stmtInsertMapset.run(mapset.id, mapset.status, mapset.title, mapset.artist, rankTime, mapset.creator);
    ...
    for (const map of maps) {
        ...
        if (shouldIndex) stmtInsertMapIndex.run(mapset.title, mapset.artist, map.version, map.id, map.mode);
    }
...
const saveMapset = async (mapsetId, index = true) => {
    ...
    for (const map of mapset.beatmaps) {
        diffLines.push(
            `* **${utils.rulesetKeyToName(map.mode)} ${map.stars.toFixed(2)} ★** - [${map.name}](https://osu.ppy.sh/beatmapsets/${mapset.id}#${map.mode}/${map.id})`
        );
        if (diffLines.join('\n').length > 1000) {
            diffLines.pop();
    ...
    // Broadcast
    broadcast('maps', 'new_mapset', mapset);
    // Log to Discord
    setImmediate(() => {
 ...
 */
const updateMapStatuses = async (rankedAfter = 0) => {
    try {
        const countTotalMaps = db
            .prepare(`SELECT COUNT(*) AS count FROM beatmapsets WHERE time_ranked > ?`)
            .get(rankedAfter).count;
        ...
        while (true) {
            if (Date.now() - lastLog > 1000 * 5) {
                const percentage = ((countProcessedMaps / countTotalMaps) * 100).toFixed(2);
                utils.log(`Checking saved maps for status updates (${percentage}%)...`);
                lastLog = Date.now();
            }

            // Get list of maps, one map per mapset
            // We're getting maps here because osu only has a get beatmaps endpoint, no get beatmapsets one
            const rows = db
                .prepare(
                    `SELECT
                    ...
                    WHERE map.mapset_id > ? AND mapset.time_ranked > ?
                    GROUP BY map.mapset_id
                    ORDER BY mapset_id ASC
                    LIMIT 50`
                )
                .all(lastMapsetId, rankedAfter);
            ...
            for (const row of rows) {
                if (!fetchedMapsetIds.has(row.mapset_id)) {
                    ...
                    db.prepare(`UPDATE beatmapsets SET status = 'unknown' WHERE id = ?`).run(row.mapset_id);
                    db.prepare(`UPDATE beatmaps SET status = 'unknown' WHERE mapset_id = ?`).run(row.mapset_id);
                }
            }
    ...
    try {
        ...
        if (existingUser?.name !== user.username) {
            ...
            db.transaction(() => {
                ...
                db.prepare(`INSERT OR REPLACE INTO users_search (rowid, names) VALUES (?, ?)`).run(
                    user.id,
                    names.join(' ')
                );
            })();
        ...
        db.prepare(`INSERT OR IGNORE INTO country_names (code, name) VALUES (?, ?)`).run(
            user.country.code,
            user.country.name
        );
        // Create/update team entry
        if (user.team?.id) {
            db.prepare(`INSERT OR REPLACE INTO teams (id, name, name_short, flag_url) VALUES (?, ?, ?, ?)`).run(
                user.team.id,
                user.team.name,
        ...
        // Get fresh user entry
        const freshUserEntry = db.prepare(`SELECT * FROM users WHERE id = ?`).get(userId);
        const userProfile = apiRead.getUserProfile(userId);
        // Broadcast
        broadcast(`user_${freshUserEntry.id}`, 'update_profile', userProfile);
        // Return fresh user entry
        return freshUserEntry;
    } catch (error) {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            for (const cat of statCategories.definitions) {
                ...
                // Update totals
                if (!catTotals[cat.id]) catTotals[cat.id] = { count: 0, seconds: 0 };
                catTotals[cat.id].count++;
                catTotals[cat.id].seconds += row.duration_secs;

                // Update yearly
                if (!catYearly[cat.id]) catYearly[cat.id] = {};
                if (!catYearly[cat.id][row.year]) catYearly[cat.id][row.year] = { count: 0, seconds: 0 };
        ...
        db.transaction(() => {
            for (const cat of statCategories.definitions) {
                ...
                stmtMain.run(
                    ...
                    totals.seconds,
                    statsOld.best_rank,
                    statsOld.best_rank_time,
                    statsOld.best_percentage_completed,
                    statsOld.best_percentage_completed_time
                );
        ...
        // Collect newly updated stats
        const catStatsNew = {};
        const catStatsYearlyNew = {};
        for (const cat of statCategories.definitions) {
            catStatsNew[cat.id] = apiRead.getUserCompletionStats(userId, cat.id);
            catStatsYearlyNew[cat.id] = apiRead.getUserYearlyCompletionStats(userId, cat.id);
        }
        ...
        for (const cat of statCategories.definitions) {
            ...
            const totals = catTotals[cat.id] || { count: 0, seconds: 0 };
            const statsOld = catStatsOld[cat.id];
            const statsNew = catStatsNew[cat.id];
            ...
            if (
                statsNew.percentage_completed > 0 &&
                (statsOld.best_percentage_completed === 0 ||
                    statsNew.percentage_completed >= statsOld.best_percentage_completed)
            ) {
        ...
        if (!IS_GLOBAL && user.is_default_category_auto) {
            let selectedId = user.default_category;
            if (!catStatsNew[user.default_category]) selectedId = 'osu-ranked';
            let selectedXp = catStatsNew[selectedId].xp;
            let selectedPercent = catStatsNew[selectedId].percentage_completed;
            for (const [catId, stats] of Object.entries(catStatsNew)) {
                // Don't consider all modes categories
                if (catId.startsWith('all')) continue;

                // Update our selection if this category has at least 10% more maps completed
                // and a completion percentage at least relatively close to our selection
                // The latter check is so we don't select a category that has half the completion but more plays,
                // which is likely something the user would rather not show off compared to the other category
                // that has a much higher completion percentage
                if (stats.xp >= selectedXp + selectedXp * 0.1 && stats.percentage_completed >= selectedPercent - 25) {
                    selectedId = catId;
                    selectedXp = stats.xp;
                    selectedPercent = stats.percentage_completed;
                }
            }

            // Update user default if it changed
            if (user.default_category !== selectedId) {
                db.prepare(`UPDATE users SET default_category = ? WHERE id = ?`).run(selectedId, user.id);
                utils.log(`Updated ${user.name}'s default category to ${selectedId}`);
            }
        }

        // Broadcast about stat changes
        broadcast(`user_${user.id}`, 'update_stats');

        utils.log(`Updated stats in ${statCategories.definitions.length} categories for ${user.name}`);

        // Check for and log milestones
        const userBaseUrl = `${env.BASE_URL}/u/${user.id}`;
        for (const cat of statCategories.definitions) {
            if (IS_GLOBAL) continue;
            ...
            const statsNew = catStatsNew[cat.id];
            const yearlyNew = catStatsYearlyNew[cat.id];
            ...
            const categoryFields = [
                { name: 'rank', value: `#${utils.formatNumber(statsNew.rank)}`, inline: true },
                { name: 'completion xp', value: utils.formatNumber(statsNew.xp), inline: true },
                {
                    name: 'maps passed',
                    value: utils.formatNumber(statsNew.count_completed),
                    inline: true
                }
            ];
            ...
            if (totalPercentStepNew > totalPercentStepOld) {
                milestoneEmbeds.push(
                    makeEmbed(`Reached ${totalPercentStepNew}% completion in ${categoryName}!`, categoryFields)
                );
            }
            ...
            if (totalPassCountStepNew > totalPassCountStepOld) {
                milestoneEmbeds.push(
                    makeEmbed(
                        `Reached ${utils.formatNumber(totalPassCountStepNew)} passes in ${categoryName}!`,
                        categoryFields
                    )
                );
            }
            ...
            if (totalCxpStepNew > totalCxpStepOld) {
                milestoneEmbeds.push(
                    makeEmbed(`Reached ${utils.formatNumber(totalCxpStepNew)} cxp in ${categoryName}!`, categoryFields)
                );
            }
            ...
            for (const statsNew of yearlyNew) {
                ...
                const isThisYear = statsNew.year === new Date().getFullYear();

                // Check category yearly completion percentage milestones
                const yearlyPercentStepOld = utils.floorToNearest(
                    statsOld.time_percentage_completed,
                    yearlyPercentStep
                );
                const yearlyPercentStepNew = utils.floorToNearest(
                    statsNew.time_percentage_completed,
                    yearlyPercentStep
                );
                if (yearlyPercentStepNew > yearlyPercentStepOld) {
                    milestoneEmbeds.push(
                        makeEmbed(
                            `Reached ${yearlyPercentStepNew}% ${statsNew.year} completion${isThisYear ? ' (for now)' : ''} in ${categoryName}!`,
                            fields
                        )
                    );
                }
                ...
                if (yearlyPassCountStepNew > yearlyPassCountStepOld) {
                    milestoneEmbeds.push(
                        makeEmbed(
                            `Reached ${utils.formatNumber(yearlyPassCountStepNew)} ${statsNew.year} passes in ${categoryName}!`,
                            fields
                        )
                    );
                }
...
const overrideUserDefaultCategory = (userId, categoryId) => {
    try {
        const user = apiRead.getUserProfile(userId);
        if (!user) throw new Error(`User ${userId} not found`);
        db.prepare(`UPDATE users SET default_category = ?, is_default_category_auto = 0 WHERE id = ?`).run(
            categoryId,
            userId
        );
        utils.log(`Overrode ${user.name}'s default category to ${categoryId} and disabled auto-selection`);
    } catch (error) {
        console.error(`Error overriding user default category:`, error);
    }
};
...
const importUser = async (userId, doFullImport = false) => {
    ...
    try {
        ...
        let beatmapsCount = 0;
        if (doFullImport) beatmapsCount = db.prepare(`SELECT COUNT(*) AS count FROM beatmaps`).get().count;
        ...
        while (true) {
            ...
            for (const ruleset_id of rulesetIds) {
                ...
                const res = await osu.getUserBeatmapsPassed(userId, {
                    ...
                    // If we included them, a pass on a convert would appear as a standard pass
                ...
                const transaction = db.transaction(() => {
                    for (const map of res.beatmaps_passed) {
                        ...
                        const existingMap = db
                            .prepare(`SELECT * FROM beatmaps WHERE id = ? AND mode = ? LIMIT 1`)
                            .get(map.id, modeKey);
                        ...
                        const time = Date.now();
                        db.prepare(
                            `INSERT OR IGNORE INTO user_passes (user_id, map_id, mapset_id, mode, time_passed) VALUES (?, ?, ?, ?, ?)`
                        ).run(user.id, map.id, map.beatmapset_id, modeKey, time);
                        passCount++;
                    }
                });
                transaction();
            ...
            // Update user's times
            db.prepare(`UPDATE users SET last_pass_time = ? WHERE id = ?`).run(Date.now(), userId);

            // Log
            const scoresPerMinute = Math.round((beatmapsOffset / (Date.now() - timeStarted)) * 1000 * 60);
            utils.log(
                `[Importing ${percentComplete}%] Saved ${passCount} new passes for ${user.name} (${scoresPerMinute} scores/min)`
            );
            broadcast(`user_${user.id}`, 'import_progress', {
                percent_complete: parseFloat(percentComplete),
                count_passes_imported: passCount
            });
        ...
        db.prepare(`UPDATE users SET last_import_time = ?, has_full_import = ? WHERE id = ?`).run(
            Date.now(),
            doFullImport ? 1 : 0,
            userId
        );
        ...
        const scoresPerMinute = Math.round((beatmapsOffset / (Date.now() - timeStarted)) * 1000 * 60);
        utils.log(
            `Completed import of ${passCount} passes for ${user.name} in ${utils.secsToDuration(Math.round(importDurationMs / 1000))} (${scoresPerMinute} scores/min)`
        );
            ...
            embeds: [
                {
                    author: {
                        name: user.name,
                        icon_url: user.avatar_url,
                        url: `${env.BASE_URL}/u/${user.id}`
                    },
                    title: `Completed ${doFullImport ? 'full ' : ''}import of ${utils.formatNumber(passCount)} passes`,
                    description: [
                        `Took ${utils.secsToDuration(Math.round(importDurationMs / 1000))} to check ${utils.formatNumber(beatmapsOffset)} beatmaps (${scoresPerMinute} scores/min)`,
...
const savePassesFromScores = async scores => {
    try {
        ...
        // Get what map data we can from the database
        const validStatuses = ['ranked', 'loved', 'approved'];
        const maps = {};
        const dbMapIds = Array.from(mapIds);
        while (dbMapIds.length > 0) {
            const ids = dbMapIds.splice(0, 50);
            const mapRows = db
                .prepare(
                    `SELECT map.*, mapset.artist, mapset.title, mapset.status AS mapset_status, mapset.time_ranked
                    FROM beatmaps map
                    JOIN beatmapsets mapset ON map.mapset_id = mapset.id
                    WHERE map.id IN (${ids.map(() => '?').join(',')})`
                )
                .all(...ids);
            for (const row of mapRows) {
                const data = {
                    ...
                    mapset_status: row.mapset_status,
                    title: row.title,
                    artist: row.artist,
                    name: row.name,
        ...
        // Fetch missing maps or those we need to get updated details for
        const missingMapIds = [];
        for (const mapId of mapIds) {
            const map = maps[mapId];
            if (!map || !validStatuses.includes(map.mapset_status)) {
                missingMapIds.push(mapId);
            }
        }
        while (missingMapIds.length > 0) {
            const ids = missingMapIds.splice(0, 50);
            utils.log(`Fetching data for ${ids.length} missing beatmaps...`);
            const res = await osu.getBeatmaps({ ids });
            for (const map of res.beatmaps) {
                const data = {
                    id: map.id,
                    ...
                    mapset_status: map.beatmapset.status,
                    title: map.beatmapset.title,
                    artist: map.beatmapset.artist,
                    name: map.version,
                    duration_secs: map.total_length,
                    saved: false
                };
                maps[map.id] = data;
            }
        ...
        // Save unsaved maps that have valid statuses
        const savedMapsetIds = [];
        for (const mapId in maps) {
            const map = maps[mapId];

            const isUnsavedRanked = !map.saved && validStatuses.includes(map.mapset_status);
            const isDuplicate = savedMapsetIds.includes(map.mapset_id);

            if (!isDuplicate && isUnsavedRanked) {
                savedMapsetIds.push(map.mapset_id);
                await saveMapset(map.mapset_id, !map.saved);
            }
        }

        // Update global stats if we saved any mapsets
        if (savedMapsetIds.length > 0) {
            updateUserCategoryStats(0);
        }

        // Loop through each affected user to process scores
        utils.log(
            `Processing and saving passes from ${relevantScoreCount} relevant (of ${scores.length} total) scores...`
        );
        const savedPasses = [];
        for (const userId in scoresByUser) {
            ...
            const transaction = db.transaction(() => {
                for (const score of scoresByUser[userId]) {
                    ...
                    const map = maps[mapId];
                    if (!map) {
                        utils.log(`Warning: Data for beatmap ${mapId} couldn't be acquired`);
                        continue;
                    }
                    ...
                    const status = map.map_status;
                    const mapsetId = map.mapset_id;
                    ...
                    const existingPass = db
                        .prepare(`SELECT * FROM user_passes WHERE user_id = ? AND map_id = ? AND mode = ? LIMIT 1`)
                        .get(userId, mapId, mode);

                    //console.log(map.map_status, map.title, '-', map.name, JSON.stringify(existingPass));
                    if (existingPass) continue;

                    // Save pass
                    db.prepare(
                        `INSERT INTO user_passes (user_id, map_id, mapset_id, mode, time_passed) VALUES (?, ?, ?, ?, ?)`
                    ).run(userId, mapId, mapsetId, mode, time);
                    savedPasses.push({
                        userId,
                        userName: user.name,
                        mapsetId,
                        mapId,
                        mode,
                        mapName: `${map.artist} - ${map.title} [${map.name}]`,
                        xp: apiRead.secsToXp(map.duration_secs)
                    });
                    newCount++;
                }
        ...
        if (savedPasses.length > 0) {
            let lines = [];
            const send = () =>
                utils.sendDiscordMessage(env.PASS_FEED_DISCORD_CHANNEL_ID, { content: lines.join('\n') });
            for (const pass of savedPasses) {
                const userLink = `[${pass.userName}](<${env.BASE_URL}/u/${pass.userId}>)`;
                const mapLink = `[${pass.mapName}](<https://osu.ppy.sh/beatmapsets/${pass.mapsetId}#${pass.mode}/${pass.mapId}>)`;
                const cxp = utils.formatNumber(pass.xp);
                const mode = utils.rulesetKeyToName(pass.mode, true);
                const line = `-# * **${userLink}** gained **${cxp} cxp** from ${mapLink} **(${mode})**`;
                lines.push(line);
                if (lines.join('\n').length > 1900) {
                    lines.pop();
                    await send();
...
const savePassesFromScoreCache = async () => {
    try {
        ...
        const cursor =
            db.prepare(`SELECT value FROM misc WHERE key = 'score_cache_cursor'`).get()?.value || defaultCursor;
        // Fetch the scores
...
const savePassesFromGlobalRecents = async () => {
    try {
        ...
        for (const mode of modes) {
            const cursor =
                db.prepare(`SELECT cursor FROM global_recents_cursors WHERE mode = ?`).get(mode)?.cursor || null;
        ...
        for (const mode of modes) {
            ...
            db.prepare(`INSERT OR REPLACE INTO global_recents_cursors (mode, cursor) VALUES (?, ?)`).run(
                mode,
                newCursor
            );
        }
...
const queueUserForImport = async (userId, full = false) => {
    try {
        ...
        const playcountsCount = full ? beatmapCount : user?.beatmap_playcounts_count || 0;
...
const saveAnalytics = async () => {
    try {
        ...
        const stmtInsertAnalytics = db.prepare(
            `INSERT OR REPLACE INTO analytics (date, metric, value) VALUES (?, ?, ?)`
        );
        const transaction = db.transaction(() => {
...
const generateSitemap = async outputPath => {
    ...
    // Select user data
    const userEntries = db.prepare(`SELECT * FROM users`).all();
    // Filter active/relevant user categories
    const userPages = [];
    for (const entry of userEntries) {
        userPages.push({
            id: entry.id,
            category_id: entry.default_category,
    ...
    const xml = await ejs.renderFile(path.join(env.ROOT, 'apps/web/views/sitemap.ejs'), {
        ...
        categoryIds,
        dayjs,
        staticPaths: ['', 'tos', 'privacy', 'changelog', 'faq', 'search', 'activity']
    });
...
const writeMiscData = (key, value) => {
    db.prepare(`INSERT OR REPLACE INTO misc (key, value) VALUES (?, ?)`).run(key, value);
};
