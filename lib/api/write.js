...
const savePassesFromScores = async scores => {
    try {
        ...
        const minRankedTime = Date.now() - 1000 * 60 * 60 * 24 * 7;
        ...
        while (dbMapIds.length > 0) {
            ...
            for (const row of mapRows) {
                const data = {
                    ...
                    is_stale: row.time_ranked > minRankedTime,
                    saved: true
                };
                maps[row.id] = data;
            }
        ...
        for (const mapId of mapIds) {
            ...
            if (!map || !validStatuses.includes(map.mapset_status) || map.is_stale) {
        ...
        while (missingMapIds.length > 0) {
            ...
            for (const map of res.beatmaps) {
                ...
                // Skip if map is already saved and its status hasn't changed
                const savedMap = maps[map.id];
                if (savedMap && savedMap.mapset_status === data.mapset_status) {
                    maps[map.id].is_stale = false;
                    continue;
                }
                maps[map.id] = data;
            }
        ...
        for (const mapId in maps) {
            ...
            const isSavedStale = map.saved && map.is_stale;
            const isDuplicate = savedMapsetIds.includes(map.mapset_id);

            if (!isDuplicate && (isUnsavedRanked || isSavedStale)) {
