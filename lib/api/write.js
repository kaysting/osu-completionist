    ...
    try {
        ...
        if (!IS_GLOBAL) {
            ...
            for (const [catId, stats] of Object.entries(catStatsNew)) {
                // I was wondering why it would unselect when selecting all mode.
                
                // Don't consider all modes categories
                // if (catId.startsWith('all')) continue;

                // Cool assumption, however rather than using xp or percentage completed as metrics for which to select,
                // instead it should be enough that the default category is only changable on your profile,
                // and not in any other page since on those we can assume they did it out of curiousity.
