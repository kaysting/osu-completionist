...
const utils = {
    ...
    secsToDuration: (secs, rounded = false, include = ['s', 'm', 'h', 'd']) => {
        ...
        // Return rounded
        if (rounded) {
            if (secs >= 86400 && include.includes('d')) return `${utils.formatNumber(secs / 86400, 1)} days`;
            if (secs >= 3600 && include.includes('h')) return `${utils.formatNumber(secs / 3600, 1)} hours`;
            if (secs >= 60 && include.includes('m')) return `${utils.formatNumber(secs / 60)} mins`;
            return `${secs} secs`;
        }
        ...
        if (include.includes('s') && secs > 0) {
            ...
            parts.push(`${utils.formatNumber(Math.floor(secs))}s`);
        }
