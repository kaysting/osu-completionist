...
const utils = {
    ...
    secsToDuration: (secs, rounded = false, include = ['s', 'm', 'h', 'd']) => {
        // 0. Handle 0 explicitly or it returns empty string
        if (secs === 0) return rounded ? '0 secs' : '0s';

        // 1. If rounded, just return the largest unit immediately (Simple logic)
        if (rounded) {
            if (secs >= 86400 && include.includes('d')) return `${(secs / 86400).toFixed(1)} days`;
            if (secs >= 3600 && include.includes('h')) return `${(secs / 3600).toFixed(1)} hours`;
            if (secs >= 60 && include.includes('m')) return `${(secs / 60).toFixed(1)} mins`;
            return `${secs} secs`;
        }

        // 2. Standard Logic: Calculate components using Integer Math
        const parts = [];
    ...
    generateJWT: (payload, expiresIn = '30d') => {
        const { readMiscData } = require('#api/read.js');
        return jwt.sign(payload, readMiscData('jwt_secret'), { expiresIn });
    },

    verifyJWT: (token) => {
        try {
            const { readMiscData } = require('#api/read.js');
            return jwt.verify(token, readMiscData('jwt_secret'));
        } catch (err) {
            return null;
        }
    },
